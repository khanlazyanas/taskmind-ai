...
export default function ChatAssistant() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {/* Premium Chat Window */}
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-[calc(100vw-3rem)] sm:w-96 h-[500px] bg-white/95 dark:bg-zinc-950/95 backdrop-blur-2xl rounded-[2rem] border border-zinc-200/50 dark:border-zinc-800/50 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 max-w-sm sm:max-w-none">
          
          {/* Glassmorphism Header */}
          <div className="bg-transparent p-5 flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800/60 flex-shrink-0">
            ...
            <button onClick={() => setIsOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-all flex-shrink-0">
              <X className="w-4 h-4" />
            </button>
          </div>

          {/* Messages Area */}
          <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-5 bg-zinc-50/50 dark:bg-zinc-900/30 text-sm">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                ...
                <div className={`p-3.5 rounded-2xl max-w-[85%] whitespace-pre-wrap leading-relaxed shadow-sm ${
                  msg.role === "user" 
          ...
          {/* Input Area */}
          <form onSubmit={sendMessage} className="p-3 sm:p-4 bg-transparent border-t border-zinc-100 dark:border-zinc-800/60 flex-shrink-0">
            <div className="flex items-center gap-1.5 sm:gap-2 bg-white dark:bg-zinc-900 p-1.5 rounded-full border border-zinc-200 dark:border-zinc-800 shadow-sm focus-within:ring-2 focus-within:ring-zinc-900 dark:focus-within:ring-zinc-100 focus-within:border-transparent transition-all w-full">
              <input
                type="text"
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Ask AI anything..."
                className="flex-1 bg-transparent px-3 sm:px-4 py-2 text-[13px] sm:text-sm font-medium focus:outline-none dark:text-white placeholder:text-zinc-400 min-w-0"
              />
              
              <button
                type="button"
                onClick={startListening}
                className={`w-8 h-8 sm:w-9 sm:h-9 flex-shrink-0 flex justify-center items-center rounded-full transition-colors ${
                  isListening
              ...
              <button
                ...
                className="flex-shrink-0 bg-zinc-950 dark:bg-white hover:bg-zinc-800 dark:hover:bg-zinc-200 text-white dark:text-zinc-950 w-8 h-8 sm:w-9 sm:h-9 flex justify-center items-center rounded-full transition-colors disabled:opacity-50"
              >
      ...
      <div className="relative">
        <button
          ...
          className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgba(255,255,255,0.1)] flex items-center justify-center transition-all duration-300 hover:scale-105 active:scale-95 border border-zinc-200/50 dark:border-zinc-700/50 z-50 relative ${
            isOpen 
